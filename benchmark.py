...
class Synthesizer:
    ...
    def synthesize(
        self, file_path: str, timeout: int = None
    ) -> tuple[ResultType, str, float]:
        ...
        try:
            start = time.time()
            result = check_output(command, stderr=STDOUT, timeout=timeout).decode(
                "utf-8"
            )
            duration = time.time() - start
            return (
                ResultType.NO_RESULT if result == NO_RESULT else ResultType.SOLVED,
                result,
                duration,
            )
...
def benchmark_all_algorithms(algorithms: list, depth: int = 12, timeout: int = 10):
    ...
    examples.sort()
    synthesizers = [
        Synthesizer(algo=algo, depth=depth, print_tree=True) for algo in algorithms
    ]

    with alive_bar(len(examples), enrich_print=False) as bar, open(
        "benchmark.csv", "w", newline=""
    ) as csvfile:
        ...
        for file in examples:
            ...
            for algo, synthesizer in zip(algorithms, synthesizers):
                ...
                result_type, result, time = synthesizer.synthesize(
                    f"{EXAMPLES_DIR}/{file}", timeout=timeout
                )
                expanded_nodes = (
                    get_expanded_nodes() if result_type != ResultType.TIMEOUT else None
                )
...
benchmark_all_algorithms(["dfs", "bfs", "best", "rules"])
